#!/usr/bin/env bash
# Preflight for hhm_rpp_philips — validates the environment the NEXT run will
# actually use. Fleet paradigm (data_acquisition/docs/migration_CLAUDE.md);
# adapted from hhm_rpp_siemens. A clean run reports ZERO warnings: treat a
...
if [ -d "$LOG_DIR" ] && [ -w "$LOG_DIR" ]; then
    ...
    # This cannot self-heal at run time: the gosu entrypoint is baked into
    # ge's shared image and does no log-dir repair, so a missing bind source
    # would be created root-owned by Docker and the run dies EACCES.
    case "$LOG_DIR" in
...
if [ ! -f .env ]; then
    ...
    for key in $REQUIRED; do
        ...
        if [ -z "$v" ]; then
            ...
            case "$key" in
                *PW*|*PASSWORD*|*SECRET*) ok ".env: $key set (masked)" ;;
                *) ok ".env: $key=$v" ;;
            esac
        fi
    done
    ...
    # VNS3_IP/VNS3_PW are deliberately NOT in this list: kept in .env by owner
    # decision 2026-08-26 (keep-and-document, CLAUDE.md KNOWN WARTS). The
    # Azure PG_* block is commented out, so ^KEY= cannot match it.
    for retired in LOGGER RUN_ENV RUN_USER DATA_STORE_STAGING TUNNEL_RESET_APP DEV_HHM_FILES STAGING_HHM_FILES PROD_HHM_FILES PG_HOST PG_PORT PG_DB PG_USER PG_PW DOCKER_GID UID_0 UID_1 UID_2; do
...
for d in utils/db utils/logger data_acquisition jobs parse persist processing read redis util; do
