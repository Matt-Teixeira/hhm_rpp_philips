#!/usr/bin/env bash
# Release hhm_rpp_philips: mirror THIS working tree to /opt/apps/<APP_NAME>,
# apply the #RELEASE: .env overrides, stamp the released commit, install deps
# as svc. Fleet paradigm — adapted from hhm_rpp_siemens' build-release.sh
# (docs/migration_CLAUDE.md Part 1: "Clean-tree guard", "Release provenance").
#
# ONE DELIBERATE DEVIATION: no image build. This app runs ge's shared
# hhm_rpp:${IMAGE_TAG} (CLAUDE.md "KNOWN WARTS"); step 5's build.sh only
# installs deps and verifies the image exists.
#
# Flow:
...
#   5. chown + deps as svc   — in-tree node_modules owned by svc
set -euo pipefail
...
# be copied by tar: dev run logs (./utils/logger/logs, *-log.*.json) and the
# .gitignore's dev-junk patterns (*.log, *.txt — covers docs/cron.txt —
# *.txe, *.output, *.pem, *.crt, deltaStore, test_data). Patterns are
# deliberately NARROW: no tracked file matches any of them (verified via
# git ls-files 2026-08-26), and '*-log.*.json' cannot match package.json /
# package-lock.json. Verify exclude changes by diffing `tar -tf` output,
# never by eyeballing patterns.
sudo mkdir -p "$DEST"
...
# simple commands, but anything that mkdirs $HOME/.docker dies (verified on
# the pilot's first release: "mkdir /nonexistent: permission denied"). NEVER
...
echo "==> release complete: $DEST  commit: $GIT_SHA  (runs ge's shared hhm_rpp image)"
